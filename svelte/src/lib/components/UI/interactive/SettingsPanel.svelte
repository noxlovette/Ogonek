...
>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-milk-50 p-6 rounded-xl shadow">
    ...
    <div class="space-y-4">
      ...
      {#each ['email', 'username', 'name'] as field}
        <div class="space-y-2">
          <label 
            for={field} 
            class="block text-sm font-medium text-brick-700 capitalize"
          >
            {field}
          </label>
          <input
	...
	<section class="flex flex-col space-y-2 bg-milk-50 rounded-lg shadow p-4 justify-between">
		<H2>Telegram</H2>
<p class="align-top">
	Click the button below and type in "/start" to the bot if you want to receive notifications about new tasks
</p>
			<a href="https://t.me/fz_notif_bot" class="bg-cyan-600 hover:bg-cyan-700 transition-colors max-w-fit flex rounded-lg text-cyan-50 py-2 px-4">
