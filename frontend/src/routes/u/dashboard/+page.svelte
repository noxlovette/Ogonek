...
>
<h1 class="text-xl lg:text-2xl xl:text-3xl font-serif">{$translations[greeting][$language]}, {$user.first_name}</h1>
</div>


<div class="w-full items-center justify-center flex flex-col my-8 border-2 py-4 border-sand-900/40 rounded-lg">
	<Header>
	...
	<div class="grid gap-4 grid-cols-1 md:grid-cols-2 w-full my-4">
		...
		<a href="/u/tasks/" class="px-1 py-2 inline-flex w-1/2 justify-center bg-sand-900/10 rounded-lg">
			{task.title}
		</a>	
		{/each}
	</div>
	<div class="flex flex-col items-center space-y-2">
		...
		<a href="/u/lessons/l/{recent_lesson.id}" class="px-1 py-2 inline-flex w-1/2 justify-center bg-sand-900/10 rounded-lg">
