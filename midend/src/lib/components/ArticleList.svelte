...
<input type="text" placeholder="Search" bind:value={searchTerm} class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-tealdeuterium-500 focus:border-transparent" />
<ul class="py-4 text-2xl font-bold">
    {#each displayedArticles as article}
        <li>
            <a href="/editor/{article.id}" class="hover:text-orangedeuterium-600"
