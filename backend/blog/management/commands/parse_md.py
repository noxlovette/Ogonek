...
class Command(BaseCommand):
    help = (
        "Parse Markdown files, populate User and Lesson models, and hash user passwords"
    )

    def handle(self, *args, **options):
        base_dir = os.path.dirname(os.path.abspath(__file__))
        folder_path = os.path.join(
            base_dir, "students_data"
        )  # Assuming students_data is in the same dir as the script
            ...
            if os.path.isdir(student_folder_path):  # Check if it's a directory
                for filename in os.listdir(student_folder_path):
                    if filename.endswith(".md"):
                        ...
                        if (
                            created or not user.has_usable_password()
                        ...
                        Lesson.objects.create(
                            assignee=user, title=filename, content=content
                        )

        self.stdout.write(
            self.style.SUCCESS(
                "Successfully processed Markdown files, created Users and Lessons, and hashed passwords."
            )
        )
