...
docker exec pg-ogonek-dev psql -U postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'pg-ogonek-dev';"

echo "🗑️  Dropping existing database..."
docker exec pg-ogonek-dev psql -U postgres -c "DROP DATABASE IF EXISTS \"pg-ogonek-dev\";"

echo "🏗️  Creating new database..."
docker exec pg-ogonek-dev psql -U postgres -c "CREATE DATABASE \"pg-ogonek-dev\" OWNER postgres;"
...
cd ../..
cargo run --bin ogonek_cli seed-db
